...
function fcitx(){
    #解决环境依赖性
    sudo dnf install fcitx fcitx-configtool fcitx-qt5 fcitx-sunpinyin fcitx-libs sunpinyin  -y # gnome-tweak-tool
    
	#开机启动fcitx
    sudo mkdir $start_dir/.config/autostart
    sudo cp $current_dir/fcitx.desktop $start_dir/.config/autostart
    #重启时生效 默认输入法修改
    sudo cp $current_dir/fcitx.sh /etc/profile.d

    #本地默认输入法
    echo 'export GTK_IM_MODULE=fcitx' >> $start_dir/.bashrc
    echo 'export QT_IM_MODULE=fcitx' >> $start_dir/.bashrc
    echo 'export XMODIFIERS=@im=fcitx' >> $start_dir/.bashrc
